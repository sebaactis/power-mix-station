...
const OptionsItems = ({ shakeCombinations, selectedShake, setSelectedShake }) => {
    return (
        <>
                    ...
                    <div className={`relative h-full bg-gradient-to-br ${shake.color} to-black`}>
                        ...
                        <div className="relative z-10 p-8">
                            <div className="flex flex-col md:flex-row justify-between items-start mb-4 gap-4 md:gap-0">
                                <h3 className={`${monaSans.className} text-2xl text-white font-extrabold italic`}>{shake.name}</h3>
                            ...
                            <p className={`${monaSans.className} text-gray-200 my-6 text-sm`}>{shake.description}</p>
                            <div className="flex flex-wrap gap-2 mb-4">
                                {shake.ingredients.map((ingredient, idx) => (
                                    <span key={idx} className={`${monaSans.className} text-xs font-bold bg-[#be185d] bg-opacity-80 text-white px-4 py-1.5 rounded-full`}>
                                        {ingredient}
                                    </span>
                                ))}
                            </div>
                            ...
                            <AnimatePresence>
                                {selectedShake === index && (
                                    ...
                                    >
                                        <div className="mb-4">
                                            <h4 className={`${monaSans.className} text-white font-bold italic my-3`}>Beneficios:</h4>
                                            <ul className={`${monaSans.className} font-extralight list-disc list-inside text-white text-sm`}>
                                                {shake.benefits.map((benefit, idx) => (
                                                    <li key={idx} className={monaSans.className}>{benefit}</li>
                                                ))}
                                            </ul>
                                        </div>
                                        <div>
                                            <h4 className={`${monaSans.className} text-white font-bold italic my-5`}>Nutrientes:</h4>
                                            {shake.nutrients.map((nutrient, idx) => (
                                                <div key={idx} className="mb-2">
                                                    <div className="flex justify-between text-xs text-gray-300 mb-1">
                                                        <span className={`${monaSans.className} text-sm`}>{nutrient.name}</span>
                                                        <span className={`${monaSans.className} text-sm`}>{nutrient.value}g</span>
                                                    </div>
