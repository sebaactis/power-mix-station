...
const AboutIntro = () => {
    return (
        <section className='bg-black py-10 md:py-36'>
            <motion.div
                className='md:-skew-x-12 flex flex-col md:flex-row m-auto md:ml-64 2xl:ml-80'
                initial={{ opacity: 0, x: 200 }}
                transition={{ duration: 0.6, delay: 0.05 }}
                viewport={{ once: true, margin: "-10% 0px" }}
                whileInView={{ opacity: 1, x: 0 }}
            >
                <article className='md:-skew-x-12 bg-cover mx-5 md:m-0 md:w-[80%] 2xl:h-[40rem] bg-center z-10' style={{ backgroundImage: `url(${maquina.src})` }} />

                <article className="flex flex-col gap-3 mx-5 md:m-0 z-20 mt-24 md:mt-0">
                    <div className='font-outline text-transparent hidden md:block text-5xl md:text-9xl ml-16 2xl:mt-28'>QUIENES SOMOS</div>
